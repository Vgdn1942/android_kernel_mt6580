	...
	PK_DBG("[_hwPowerOn]after get, powerId:%s, regVCAM: %p\n", powerId, temp );
 	...
 	if(!IS_ERR(temp)){
        ...
        if(regulator_enable(temp)!= 0) {
            ...
            regulator_put(*regVCAM);  //wenzhiquan20150723 for camera switch to front fail,old no star
            *regVCAM = NULL;  //wenzhiquan20150723 for camera switch to front fail,old no star
            return ret;
        }
        ret = true;
    }else
...
/*
    if(pinSetIdx == 0 && (0 != strcmp(currSensorName,SENSOR_DRVNAME_OV5670_MIPI_RAW))&& (0 != strcmp(currSensorName,SENSOR_DRVNAME_OV5670FF_MIPI_RAW))&& (0 != strcmp(currSensorName,SENSOR_DRVNAME_S5K5E2YA_MIPI_RAW))&&(0 != strcmp(currSensorName,SENSOR_DRVNAME_S5K5E2YAAF_MIPI_RAW))&&(0 != strcmp(currSensorName,SENSOR_DRVNAME_S5K5E8YX_MIPI_RAW)))
    	return 0;
    if(pinSetIdx == 1 && (0 != strcmp(currSensorName,SENSOR_DRVNAME_GC0339_MIPI_RAW))&& (0 != strcmp(currSensorName,SENSOR_DRVNAME_GC2355_MIPI_RAW))&& (0 != strcmp(currSensorName,SENSOR_DRVNAME_OV5670_FRONT_MIPI_RAW))&& (0 != strcmp(currSensorName,SENSOR_DRVNAME_OV2680_MIPI_RAW))&&(0 != strcmp(currSensorName,SENSOR_DRVNAME_OV5670_FRONT_SUNRISE_MIPI_RAW)))
    	return 0;
*/
...
/*
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K5E2YAAF_MIPI_RAW, currSensorName))){
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K5E8YX_MIPI_RAW, currSensorName)))
        {
		   ...
		   //VCAM_IO
            ...
            //AF_VCC
   //         if(TRUE != _hwPowerOn(CAMERA_POWER_VCAM_AF, VOL_2800,&regVCAMD))
            {
...
//                goto _kdCISModulePowerOn_exit_;
	        ...
	        PK_DBG("[CAMERA SENSOR] enable analog power (VCAM_A), power id = %s\n", CAMERA_POWER_VCAM_A);
            mdelay(1);
		...
		PK_DBG("[CAMERA SENSOR] enable digital power (VCAM_IO), power id = %s \n", CAMERA_POWER_VCAM_IO);
             mdelay(1);
	  ...
	  PK_DBG("[CAMERA SENSOR] enable digital power (RST) \n");
            mdelay(2);
            if(TRUE != _hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1200,&regVCAMD))
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power (VCAM_D), power id = %s \n", CAMERA_POWER_VCAM_D);
		...
		PK_DBG("[CAMERA SENSOR] enable digital power (VCAM_D), power id = %s \n", CAMERA_POWER_VCAM_D);
            mdelay(5);
           //AF_VCC
            if(TRUE != _hwPowerOn(CAMERA_POWER_VCAM_AF, VOL_2800,&regVCAMAF))
            {
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5670_FRONT_SUNRISE_MIPI_RAW, currSensorName))){
        #if 0
            mt_set_gpio_mode(GPIO_SPI_MOSI_PIN,GPIO_MODE_00);
            mt_set_gpio_dir(GPIO_SPI_MOSI_PIN,GPIO_DIR_OUT);
            mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ONE);
...
/*
  ...
  //          mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ZERO);
            ...
            //AF_VCC
//            if(TRUE != _hwPowerDown(CAMERA_POWER_VCAM_AF,&regVCAMIO))
            {
