...
#define AF_JUMP_BACK_STEP 30 //LINE <> <DATE20131217> <S8513A:avoid imx111 af noise.> wupingzhou
inline static int moveAF(unsigned long a_u4Position)
{
    ...
    unsigned long temp_fortPosition;//LINE <> <DATE20131217> <S8513A:avoid imx111 af noise.> wupingzhou
    ...
    else if(g_u4CurrPosition<g_u4TargetPosition &&( (g_u4TargetPosition-g_u4CurrPosition)>AF_JUMP_BACK_STEP))
    {
    	
        do
        {
            temp_fortPosition = g_u4CurrPosition+AF_JUMP_BACK_STEP;
            moveFM50AF_int(temp_fortPosition);
            mdelay(10);
        }
        while((g_u4TargetPosition-temp_fortPosition)>AF_JUMP_BACK_STEP);
        
        if(temp_fortPosition!=g_u4TargetPosition)
        {
            moveFM50AF_int(g_u4TargetPosition);
        }            
    }
