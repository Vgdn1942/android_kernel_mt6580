    ...
    for (retry = 0; retry < 5; retry++)
    {
        ...
        if (ret > 0)
        {
        	ret = gtp_i2c_read(client, buf, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
             if(ret > 0 && !memcmp(buf, config, cfg_len -1 + GTP_ADDR_LENGTH)){
                 break;
             }
        }
    }
...
void force_reset_guitar(void)
{
    ...
    // Power off TP
    #ifdef TPD_POWER_SOURCE_CUSTOM
     #ifdef CONFIG_ARCH_MT6580  // by zx 20150316
//	 #if 0
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {     
                    ...
                    )
                {
                    ...
                    input_sync(tpd->dev);
                    //switch to main overlay system
                    doze_buf[2] = 0x09;
                    i2c_write_bytes_non_dma(i2c_client_point, 0x8046, &doze_buf[2], 1);
                    i2c_write_bytes_non_dma(i2c_client_point, 0x8040, &doze_buf[2], 1);
                }
                ...
                else
                {
                    ...
                    //switch to main overlay system
                    doze_buf[2] = 0x00;
                    i2c_write_bytes_non_dma(i2c_client_point, 0x814B, &doze_buf[2], 1);
                }
            }
            mutex_unlock(&i2c_access);
            continue;
        ...
        if ((finger == 0x00) && (CHIP_TYPE_GT9F == gtp_chip_type))
        {
            ...
            GTP_DEBUG("Chip Request : %d.", rqst_data[2]);
            switch(rqst_data[2]&0x0F)
            {
                ...
                case GTP_RQST_MAIN_CLOCK:
                    GTP_INFO("Request main clock.get_boot_mode() =  %d ",get_boot_mode());
		      ret = gtp_send_cfg(i2c_client_point);
                    rqst_processing = 1;
					
					 if (RECOVERY_BOOT == get_boot_mode()) 
					{ 
					u8 gtp_clk_buf0[8] = {0x80, 0x20,0x4E, 0x4E,0x4E,0x4E,0x4E, 0x7A}; 
					//ret = i2c_write_bytes(i2c_client_point, 0x8020, gtp_clk_buf, 6);
					ret = gtp_i2c_write(i2c_client_point, gtp_clk_buf0, 8);
					ret = 1; 
					GTP_INFO("recovery_mode_main_clock_write."); 
					} 
				#ifdef CONFIG_MTK_KERNEL_POWER_OFF_CHARGING		//zhangdongfang 20150612
					 else if((KERNEL_POWER_OFF_CHARGING_BOOT == get_boot_mode())||(LOW_POWER_OFF_CHARGING_BOOT == get_boot_mode()))
				 	{
						 u8 gtp_clk_buf0[8] = {0x80, 0x20,0x4E, 0x4E,0x4E,0x4E,0x4E, 0x7A}; 
						 //ret = i2c_write_bytes(i2c_client_point, 0x8020, gtp_clk_buf, 6);
						 ret = gtp_i2c_write(i2c_client_point, gtp_clk_buf0, 8);
...
static s8 gtp_enter_doze(struct i2c_client *client)
{
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
            ...
            if(CHIP_TYPE_GT9F == gtp_chip_type)
            {
                doze_status = DOZE_PENDING;
                GTP_DEBUG("GTP wait for overlay doze!");
            }
            else
        #endif
            {
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    if (CHIP_TYPE_GT9F == gtp_chip_type)
    {
        ...
        while (retry++ < 3)
        {
            ...
            {
                GTP_DEBUG("Power key wakeup gesture doze.");
                ret = gup_recovery_from_gesture(1);
                if(FAIL == ret)
                {
                    GTP_ERROR("recovery from gesture system failed,retry:%d", retry);
                    continue;
                }
            }
            else
...
void Gt9xx_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
   ...
   // GTP_INFO("Gt9xx_tpd_get_fw_vendor_name: tpd_info.sid =  %d",tpd_info.sid);  
    ...
    switch(tpd_info.sid)
    {
        case GT9XX_CTP_VENDOR_YEJI:
            sprintf(fw_vendor_name,"%s","YEJI");
	      break;
